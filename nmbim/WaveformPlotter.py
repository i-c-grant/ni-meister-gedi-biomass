...
class WaveformPlotter:
    ...
    def plot(self, wf: Waveform):
        ...
        if self.x_lab:
            ax.set_xlabel(self.x_lab, fontsize=14)

        if self.y_lab:
            ax.set_ylabel(self.y_lab, fontsize=14)

        # Add a title based on the shot number
        shot_number = wf.get_data('metadata/shot_number')
        ax.set_title(f"Waveform processing for GEDI shot {shot_number}",
        ...
        ax.annotate(f"Biomass index: {biomass_index:.2f}",
    ...
    def save(self):
        ...
        if self.last_fig is not None and self.last_shot is not None:
            self.last_fig.savefig(f"waveform_plot_{self.last_shot}.png", dpi=600)
        else:
